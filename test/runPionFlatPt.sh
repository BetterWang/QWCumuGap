...
cp qwcumu_FlatPt_Gen.py $WORKDIR/cfg.py
cd $WORKDIR
cmsRun cfg.py
ls -l
NEW_UUID=$(openssl rand -base64 32 | tr -dc 'a-zA-Z0-9' | head -c 8)
echo /eos/cms/store/group/phys_heavyions/qwang/PbPb2015_cumu/MC/FlatPt/cumu_$NEW_UUID.root
cp cumu.root /eos/cms/store/group/phys_heavyions/qwang/PbPb2015_cumu/MC/FlatPt/cumu_$NEW_UUID.root
echo done transfter
